        ...
        tokio::spawn({
            ...
            async move {
                ...
                tokio::spawn(async move {
                    let (delivery_context, payload) = create_delivery_context(d, c);
                    match handler.call(payload, &delivery_context).await {
                        Ok(ret) => match ret.handle_result(&delivery_context).await {
                            Ok(_) => {
                                if let Err(e) = delivery_context.acker.ack(Default::default()).await
                                {
...
pub trait AMQPHandler<P, T, Err>: Send + 'static
where
    T: AMQPResult,
{
    fn call(
